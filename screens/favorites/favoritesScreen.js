...
class FavoritesScreen extends Component {
  ...
  async componentDidMount() {
    ...
    await AsyncStorage.getItem("user_id").then(async (res) => {
      await axios
        .post("http://192.168.11.65:5000/user/getFavorite", { user_id: res })
        .then(async (res1) => {
          ...
          for (let i = 0; i < reste.length; i++) {
            await axios
              .post("http://192.168.11.65:5000/user/getMechanic", {
                mechanic_id: reste[i],
              })
              .then((result) => {
                var a = {
...
const Favorites = () => {
  ...
  const deleteRow = async (rowMap, rowKey) => {
    ...
    await AsyncStorage.getItem("user_id").then(async (res) => {
      await axios
        .post("http://192.168.11.65:5000/user/deleteFavo", {
          user_id: res,
          mechanic_id: listData[0].key,
        })
        .then((rez) => {
