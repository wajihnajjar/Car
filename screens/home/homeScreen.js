...
const NearestPlaces = ({ props }) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      await _GetCord();
      await axios
        .get("http://192.168.18.22:5000/admin/getAllMechanic")
        .then((res) => {
          ...
          for (var i = 0; i < res.data.length; i++) {
            ...
            var latitude = res.data[i].cordinate.substr(
              0,
              res.data[i].cordinate.substr(
                0,
                res.data[i].cordinate.indexOf(" ")
              )
            );
            var longitude = res.data[i].cordinate.substr(
              res.data[i].cordinate.indexOf(" ") + 1
            );
            ...
            markers[i].distance = calcDistance(
              parseFloat(latitude),
